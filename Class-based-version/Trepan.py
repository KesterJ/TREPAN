...
class Trepan(object):
    ...
    def __init__(self, oracle, maxsize=10, minsamples=1000, significance=0.05, improvement=1.05):
    ...
    def fit(self, traindata, featnames=[]):
        """
        ...
        if featnames == []:
            self.featnames = [str(i) for i in range(numfeats)]
        else:
    ...
    def __create_initial_node(self, nodename):
        ...
        parent = None
        sister = None
        node = {'constraints': constraints, 'reach': reach, 'fidelity': fidelity,
                'reached': reached, 'isleaf': isleaf, 'predictedclass': predictedclass,
                'mntest': mntest, '0daughter': daughter0, '1daughter': daughter1,
                'parent': parent, 'sister': sister}
        return node
    
    
    def __create_node(self, nodename, parent, sister, passed):
        """
        ...
        reached = [self.__passes_mn_tests(self.traindata[i,:], constraints) for i in range(self.numsamples)]
        reach = sum(reached)/self.numsamples
        #Get predicted classes for samples that reach this node
    ...
    def __passes_mn_tests(self, example, constraints):
        ...
        while allpassed and counter<len(constraints):
            passed = self.__passes_mn_test(example, (constraints[counter][0], constraints[counter][1]))
    ...
    def __expand_node(self, nodename):
        ...
        mntest = self.__construct_test(nodename)
        ...
        if mntest != None:
            ...
            self.tree[daughter0] = self.__create_node(daughter0, nodename, daughter1, False)
            self.tree[daughter1] = self.__create_node(daughter1, nodename, daughter0, True)
    ...
    def __draw_sample(self, nodename):
        ...
        if samplesneeded > 0:
            ...
            for feat in range(self.numfeats):
                ...
                if stats.ks_2samp(localsamples[:,feat], parentsamples[:,feat])[1] <= self.significance/self.numfeats:
                    ...
                    if len(uniques)==1:
                        ...
                        kernels[feat] = stats.gaussian_kde(localsamples[:,feat], bw_method=bandwidth)
                else:
    ...
    def __draw_instances(self, number, kernels, constraints, singlevalsdict):
        ...
        instances = np.array([self.__draw_instance(kernels, probslist, feattests, singlevalsdict) for i in range(number)])
        return instances
    ...
    def __binary_info_gain(self, feature, threshold, samples, labels):
        ...
        #half of split)
        afterent = (self.__entropy(labels[split1])*(sum(split1)/len(labels)) + 
                    self.__entropy(labels[split2])*(sum(split2)/len(labels)))
                        ...
                        for threshold in tests[feature]:
                            ...
                            for greater in [True, False]:
                                ...
                                for incrementm in [True, False]:
                                    ...
                                    newtest = self.__expand_mofn_test(test, feature, threshold, greater, incrementm)
                                    #Get info gain and compare it
                                    gain = self.__mofn_info_gain(newtest, samples, labels)
                                    #Compare gains
                                    if (gain > min(currentgains)) and (gain > self.improvement*currentbest):
    ...
    def __prune_tree(self):
        ...
        while altered:
            ...
            for node in nodes:
                ...
                if (self.tree[node]['isleaf'] and self.tree[sister]['isleaf']
                    and self.tree[node]['predictedclass']==self.tree[sister]['predictedclass']):
    ...
    def draw_tree(self, filename):
        ...
        def write_test(test):
            ...
            for subtest in test[1]:
                ...
                substr = "%s %s %f\\n "%(featnames[subtest[0]], symbol, subtest[1])
